...
var debug = /localhost[:]\d+|pizzahuteventtest.azurewebsites.net/.test(location.href);
...
$(function(){
    ...
    // $('header .menu-open .close').on('click', function(){
    // 	$('header .menu-open').removeClass('on');
    // });


    $(window).on('resize', function(){
    	$('iframe[data-src]').each(function(i,d ){
    		var ww = $(window).width();
    		if(ww > 800 && !$(this).hasAttr('src')){
...
var share = {
	facebook: function(href, title){
		location.href = 'https://www.facebook.com/dialog/feed?app_id='+app.appId+'&display=popup&caption=%E5%BF%85%E5%8B%9D%E5%AE%A2%20%E2%80%93%20%E8%B1%90%E5%AF%8C%E7%94%9F%E6%B4%BB%20%E6%94%BE%E5%A4%A7%E6%BB%BF%E8%B6%B3&link=http://www.pizzahutevent.com.tw/&redirect_uri='+location.href;
	},
	googleplus: function(href){
		href = encodeURIComponent(href || location.href + '?utm_source=g+&utm_medium=googleplus_m&utm_campaign=');
	...
	email: function(href, title){
		href = encodeURIComponent(href || location.href + '?utm_source=email&utm_medium=email_m&utm_campaign=');
...
window.fbAsyncInit = function() {
	app.appId = debug ? '262198664233320' : '574874969335972';
	FB.init({
...
app.partial.spa = function(){
	...
	function updateContent(uri, name, menu, callback, isPopstate){
		...
		$.ajax({
			...
			response = response.replace('<html', '<div').replace('</html>', '</div>');
			response = response.replace('<body', '<item').replace('</body>', '</item>');
			bodyClass = $('item', response).attr('class');
			$('body').attr('class', bodyClass);	

			if(uri === '/'){
				container.trigger('page:update:home', menu);
			}else{
				container.trigger('page:update:' + name, menu);
			}
			container.trigger('page:update', name);	
	...
	$('a[data-href]').on('click', app.spaButtonClick);
...
app.partial.preload = function(){
	...
	function imageReload(callback){
		...
		function checkAll(src){
			...
			$('.percent').text(Math.round(i*100/sum) + '%');
			if(alldone){
				//全部圖片下載完成
				imageLoaded();
			}
...
app.partial.ga = function(){
	...
	window.onYouTubeIframeAPIReady = function () {
		...
		fragTick = setInterval(function(){
			var playing = $('#tvc iframe[src]');
			if(playing.length && playing.getDuration && playing.getCurrentTime){
				$('#tvc iframe[src]').attr('id','yttvc');
				playing = new YT.Player('yttvc',{
					events:{
						onStateChange: function(){
							ga('send', 'event', 'Button', 'click', 'tvcplay');
						}
					}
				});
				...
				if(played >= 10 && played < 25 && !percentSend.view10){
					ga('send', 'event', 'Button', 'click', '10%');
					percentSend.view10 = true;
				}
				if(played >= 25 && played < 50 && !percentSend.view25){
					ga('send', 'event', 'Button', 'click', '25%');
					percentSend.view25 = true;
				}else if(played >= 50 && played < 75 && !percentSend.view50 ){
					ga('send', 'event', 'Button', 'click', '50%');
					percentSend.view50 = true;

				}else if(played >= 75 && played < 90 && !percentSend.view75){
					ga('send', 'event', 'Button', 'click', '75%');
					percentSend.view75 = true;
				}else if(played >= 90 && !percentSend.view90){
					ga('send', 'event', 'Button', 'click', '90%');
					percentSend.view90 = true;
				}
			}		
		}, 500);

		app.youtubeReady = true;
		$('#kv iframe[src]').attr('id','ytkv');
		app.ytkv = new YT.Player('ytkv',{});
	};
	var container = $('#container');
	var pageView = {
		'home': 'mobilehome',
		...
		'smoking': 'puzzle_m',
		'puzzle': 'puzzle2_m',
		'thankyou': 'sharepage_m',
	};
	container.on('page:update', function(e, name){
		console.log(pageView[name]);
		// alert(name);
		...
		if(ismobile){
			ga('send', 'pageview', { 'page': pageView[name], 'title': pageView[name]});
		}else{
			ga('send', 'pageview', { 'page': 'home', 'title': 'home'});
		}

		$('[data-ga-click][data-ga-pc]').on('click', function(){
			if(!ismobile){
				ga('send', 'event', 'Button', 'click', $(this).attr('data-ga-click'));
			}
		}).removeAttr('data-ga-pc');

		$('[data-ga-mobile-click][data-ga-mobile]').on('click', function(){
			if(ismobile){
				ga('send', 'event', 'Button', 'click', $(this).attr('data-ga-mobile-click'));
			}
		}).removeAttr('data-ga-mobile');

		var scrolled = false;
		$(window).on('scroll', function(){
			if(!scrolled){
				ga('send', 'event', 'Button', 'click', ismobile ? 'scroll_m' : 'Scroll');
			}
			scrolled = true;
...
'use strict';
/* jshint devel:true, latedef:false,unused: false  */
/*global app, FB, YT, TweenMax */
app.partial.home = function(){
	var container = $('#container');
	container.on('page:update:home', function(){	
		// $('.home .create .submit').unbind('click').on('click', app.spaButtonClick);
		var scrollTop = 0;
		var activeSection = '';
		var range = app.range = {};
		var $page = $(this);

		$('.home .create form').unbind('submit').on('submit', function(e){
			e.stopPropagation();
			e.preventDefault();
			return false;
		} );
		$('.submit', $page).unbind('click').one('click', function(e){
			if(isNaN($('select option:selected', $page).val() * 1)){
				alert('請選擇人數');
				container.trigger('page:update:home');
				return false;
			}
			...
			$.ajax({
				url: 'api-create',
				data:{
					count: $('select option:selected', $page).val() * 1
				},
				method: 'post',
				success: function(r){
					app.puzzle = {
		...
		$('.join', $page).unbind('click').on('click', app.spaButtonClick);
		$('.home .sprite').addClass('play');

		$('body.home .range').each(function(){
			...
			range[id].middle = function(){
				if($('#' + id).height() > $(window).height()){
					return $('#' + id).offset().top + $(window).height() / 2;
				}else{
		...
		$(window).on('scroll resize', function(){
			...
			var currentTop = $(window).scrollTop() + $(window).height() / 5;
			var currentButt = $(window).scrollTop() + $(window).height() / 5 * 4;
			$('body.home .range').each(function(i, section){
				...
				if(activeSection && !$('#' + activeSection).hasClass('on')){
					...
					switch(activeSection){
						case 'kv':
							$('.scroll').addClass('sec1').removeClass('sec2').removeClass('sec3');
							$('header .tagline').addClass('fade').removeClass('in');
							if(app.youtubeReady && app.ytkv.playVideo){app.ytkv.playVideo();}
							break;
						case 'tutorial':
							$('.scroll').removeClass('sec1').addClass('sec2').removeClass('sec3');
							$('header .tagline').addClass('fade in');
							if(app.youtubeReady && app.ytkv.pauseVideo){app.ytkv.pauseVideo();}
							break;
						case 'tvc':
							$('.scroll').removeClass('sec1').removeClass('sec2').addClass('sec3');
							$('header .tagline').addClass('fade in');
							if(app.youtubeReady && app.ytkv.pauseVideo){app.ytkv.pauseVideo();}
							break;
						case 'rule':
							$('.scroll').removeClass('sec1').removeClass('sec2').removeClass('sec3');
							$('header .tagline').addClass('fade in');
							if(app.youtubeReady && app.ytkv.pauseVideo){app.ytkv.pauseVideo();}
							break;
					}
		...
		$('header nav.hidden-xs a[data-scrollto]').on('click', function(){
...
/* jshint devel:true, latedef:false,unused: false  */
/*global app, $ */
app.partial.join = function(){
	var container = $('#container');
	container.on('page:update:join', function(){
		var $page = $(this);	
		$('.go', $page).unbind('click').one('click', function(e){
			if($('input', $page).val().length !== 4){
				alert('請確認序號');
				container.trigger('page:update:join');
				return false;
			}	
			$('html').removeClass('loading-done');
			$.ajax({
	...
	if($('body.page-template-template-join').length){
		app.currentUri = 'join';
		container.trigger('page:update:join');
		container.trigger('page:update', 'join');
	}
...
/* jshint devel:true, latedef:false,unused: false  */
/*global app, $ */
app.partial.waiting = function(){
	var container = $('#container');
	if($('body.page-template-template-waiting').length && !app.puzzle){
		$('header .hide [ref=home]').trigger('click');
	}
	app.updateTick = app.updateTick || 0;
	var counter = 0;
	container.on('page:update:waiting', function(){
		var $page = $(this);	
		$('.empty-seat span', $page).text(app.puzzle.total - app.puzzle.current);
		$('.total-seat span', $page).text(app.puzzle.total);
		$('.hash span', $page).text(app.puzzle.uuid);
		startWaiting();
		function startWaiting(){
			clearInterval(app.updateTick);
			app.updateTick = setInterval(function(){
				$.ajax({
					url: 'api-update',
					data:{
						uuid: app.puzzle.uuid,
						ticket: app.puzzle.ticket
					},
					method: 'post',
					success: function(r){
						if(r.expireOrNotExists){
							console.log('expireOrNotExists');
							counter++;
							if(counter > 200){
								clearInterval(app.updateTick);
								$('header [ref=home]').trigger('click');
								return false;
							}else{
								counter = 0;
							}
						}else if(r.ticketExpireOrNotExists){
							console.log('ticketExpireOrNotExists');
							counter++;
							if(counter > 200){
								clearInterval(app.updateTick);
								$('header [ref=home]').trigger('click');
							...
							app.puzzle.total = r.total;
							app.puzzle.current = r.current;
							// console.log(r);
							if(app.puzzle.total - app.puzzle.current < 0){
								$('header .hide a[ref=home]').trigger('click');
							}
							...
							if(r.startPuzzle){
								app.puzzle.gameSerial = r.gameSerial;
								clearInterval(app.updateTick);
								$('header .hide a[ref=cf]').trigger('click');
							}
						}
					}
				});
...
/* jshint devel:true, latedef:false,unused: false  */
/*global app, $ */
app.partial.cf = function(){
	var container = $('#container');
	container.on('page:update:cf', function(){
		$('.cf .bg').addClass('play');
		$('.cf .bg').on('animationend', function(e){
			// console.log(e);
			if(!/debug/.test(location.search)){
				setTimeout(function(){
					$('header .hide a[ref=smoking]').eq(0).trigger('click');
				}, 1 * 1000 + 200);
			}else{
				console.log('disable redirect');
			}
		});
	});
	if($('body.page-template-template-cf').length){
		container.trigger('page:update:cf');
		container.trigger('page:update', 'cf');
	}
...
/* jshint devel:true, latedef:false,unused: false  */
/*global app, $ */
app.partial.smoking = function(){
	var container = $('#container');
	// if($('body.page-template-template-smoking').length){
	// 	$('header .hide [ref=home]').trigger('click');
	// }
	container.on('page:update:smoking', function(){		
		...
		var canvas = document.getElementById("canvas"),
			...
			loading = true;
		canvas.height = document.body.offsetHeight * 600;
		canvas.width = document.body.offsetWidth * 500;
		...
		smokeImage.src = $('.smoking img.hide').attr('src');
		...
		if(!app.puzzle){
			...
			var sn = app.puzzle.gameSerial;
			var total = app.puzzle.total;
			var img = new Image();
			img.onload = function(){
				setTimeout(function(){
					$('header .hide a[ref=puzzle]').attr('data-href', 'puzzle-'+ sn +'-in-' + total).trigger('click');
				}, 5 * 1000);
			};
			img.src = 'http://www.pizzahutevent.com.tw/api-puzzle/?piece='+sn+'&in=' + total;

		}
	});
	if($('body.page-template-template-smoking').length){
		container.trigger('page:update:smoking');
		container.trigger('page:update', 'smoking');
	}
...
/* jshint devel:true, latedef:false,unused: false  */
/*global app, $ */
app.partial.puzzle = function(){
	var container = $('#container');
	if($('body.page-template-template-puzzle').length){
		// $('header .hide [ref=home]').trigger('click');
	}
	container.on('page:update:puzzle', function(){
		var $this = this;
		setTimeout(function(){
			$('.done', $this).on('click', app.spaButtonClick).addClass('in');
		}, 8 *1000);
	});
	if($('body.page-template-template-puzzle').length){
		container.trigger('page:update:puzzle');
		container.trigger('page:update', 'puzzle');
	}
...
app.partial.thankyou = function(){
	var container = $('#container');
	container.on('page:update:thankyou', function(){
		$('.share', this).unbind('click').bind('click', function(){
			if(!app.puzzle){
				alert('已逾期請重新加入');
				$('header .hide [ref=home]').trigger('click');
				return false;
			...
			FB.login(function(r){
				FB.api('/me?field=email,name', function(me){
					$.ajax({
						...
						method: 'post',
						success: function(r){							
							var link = encodeURIComponent('http://www.pizzahutevent.com.tw/?content=gif');
							var caption = encodeURIComponent('快來一起拼必勝客鬆厚大比薩 抽日本雙人遊');

							if(!app.debug){
								location.href = 'https://www.facebook.com/dialog/feed?caption='+caption+'&app_id='+app.appId+'&display=popup&link='+link+'&redirect_uri='+location.href;
							}else{
								console.log('disable redirect');
							}
						}
		...
		// $('.message', this).unbind('click').bind('click', function(){
		// 	FB.login(function(r){
		// 		location.href = 'https://www.facebook.com/dialog/send?app_id='+app.appId+'&display=popup&redirect_uri=http://pizzahuteventtest.azurewebsites.net/thankyou';
  //           },{scope: 'email'});
		// });
	});
	if($('body.page-template-template-thankyou').length){
		container.trigger('page:update:thankyou');
		container.trigger('page:update', 'thankyou');
	}
};
